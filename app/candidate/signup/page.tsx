  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (result.needsEmailConfirmation) {
        setSuccessMessage(
          "Your account was created. Check your email to confirm and continue to the jobs portal.",
        );
      }
    } catch (signupError) {
      if (signupError instanceof Error) {
        if (signupError.message.toLowerCase().includes("429")) {
          setError(
    ...
    <main className="flex min-h-screen items-center justify-center bg-[#f4f8ff] px-6 py-10">
      <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-sm ring-1 ring-black/5">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Sign up to apply for jobs and track your interview journey.
        </p>
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          Already registered?{" "}
          <Link
            href={`/candidate/login?redirectTo=${encodeURIComponent(redirectTo)}`}
